...
#[Framework\Attributes\CoversClass(Report\WarningList::class)]
...
final class WarningListTest extends Framework\TestCase
{
    use Test\Util\Helper;

    public function testCreateReturnsWarningListWhenValuesAreEmpty(): void
    {
        $warningList = Report\WarningList::create();

        self::assertSame([], $warningList->toArray());
        self::assertEquals(Report\Count::zero(), $warningList->count());
    }

    public function testCreateReturnsWarningListWhenValuesAreNotEmpty(): void
    {
        $faker = self::faker();

        $values = \array_map(static function () use ($faker): Report\Warning {
        ...
        $warningList = Report\WarningList::create(...$values);

        self::assertSame($values, $warningList->toArray());
        self::assertEquals(Report\Count::fromInt(\count($values)), $warningList->count());
    }
}
